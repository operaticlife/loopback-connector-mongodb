...
MongoDB.prototype.toDatabase = function(model, data) {
  ...
  // var propDefs = {};
  // for (p in props) {
  //   propDefs[p] = this.getPropertyDefinition(model, p);
  // };
  if (this.settings.enableGeoIndexing !== true) {
 ...
 */
function convertDecimalProps(data, props, path) {
  if (debug.enabled) debug('convertDecimalProps props: ', util.inspect(props));
  /*  for (const p in props) {
    const prop = props[p];
    const isDecimal = checkDecimalProp(prop);
    // data[p] && prop && prop.mongodb &&
    //   prop.mongodb.dataType &&
    //   prop.mongodb.dataType.toLowerCase() === 'decimal128';
    if (isDecimal) {
      if (Array.isArray(data[p])) {
        data[p] = data[p].map(p => Decimal128.fromString(p));
      } else {
        data[p] = Decimal128.fromString(data[p]);
      }
      debug('convertDecimalProps decimal value: ', data[p]);
    }
    convertNestedDecimalProps(data, p, prop);
  } */
  console.log('incoming path ', path);
  traverse(props).map(function(node) {
    // console.log('key ', this.key);
    // console.log('lvl ', this.level);
    if (this.level === 1 && checkDecimalProp(node) === true) {
      console.log('Dkey ', this.key);
      // console.log('Dlevel', this.level);
      console.log('Dpath ', this.path);
      // console.log('parent ', this.parent);
      // console.log('leaf or no leaf', this.isLeaf);
    }
    if (this.level === 1 && checkNested(node)) {
      console.log('nested key', this.key);
      var children = node.type[0] ? node.type[0].definition.properties : node.type.definition.properties;
      convertDecimalProps(data, children, this.path);
    }
  });
  return data;
}
...
function checkNested(propDefinition) {
  ...
  if (Array.isArray(propDefinition.type)) {
    if (!propDefinition.type[0].definition) {
      ...
      if (propDefinition.type[0].definition.properties) return propDefinition.type[0].definition.properties;
    }
  } else {
    if (!propDefinition.type.definition) {
      return false;
    } else {
      if (propDefinition.type.definition.properties) return propDefinition.type.definition.properties;
    }
  }
}

function checkDecimalProp(property) {
  return property && property.mongodb &&
  property.mongodb.dataType &&
  property.mongodb.dataType.toLowerCase() === 'decimal128';
}

function convertNestedDecimalProps(data, propName, propDef) {
  const subProps = checkNested(propDef);
  if (!subProps) {
    return data;
  } else if (subProps) {
    for (var p in subProps) {
      const subPropDef = subProps[p];
      const isDecimal = checkDecimalProp(subPropDef);
      if (isDecimal) {
        if (Array.isArray(data[propName])) {
          for (var i = 0; i < data[propName].length; i++) {
            if (typeof data[propName][i][p] === 'string') {
              console.log('being converted', data[propName][i][p]);
              data[propName][i][p] = Decimal128.fromString(data[propName][i][p]);
              if (i === data[propName].length) continue;
            }
          }
          // data[propName].map((ele, idx) => data[propName][idx][this.p] = Decimal128.fromString(ele[this.p]), self);
        } else {
          data[propName][p] = Decimal128.fromString(data[propName][p]);
          continue;
        }
      }
      if (checkNested(subPropDef)) {
        convertNestedDecimalProps(data, p, subPropDef);
      }
    }
  }
}
